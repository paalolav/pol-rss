 ...
 */
export function parseJsonFeed(content, options) {
    if (!content) {
    ...
    return items.map((item) => {
        ...
        if (!imageUrl && item.attachments) {
            const imageAttachment = item.attachments.find(att => att.mime_type && att.mime_type.startsWith('image/'));
            if (imageAttachment) {
 ...
 */
export function getJsonFeedMetadata(content) {
    try {
        const feed = JSON.parse(content.trim());
        if (!feed.version || !feed.version.includes('jsonfeed.org')) {
            return null;
        }
        return {
