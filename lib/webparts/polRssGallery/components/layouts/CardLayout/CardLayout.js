 ...
 * CardLayout component
 */
export const CardLayout = ({ items, columns = 'auto', cardSize = 'md', gap = 'md', fallbackImageUrl, forceFallback = false, hideImages = false, showPubDate = true, showDescription = true, showSource = false, truncateDescription = 150, imageAspectRatio = '16:9', isLoading = false, skeletonCount = 6, onItemClick, className = '', testId = 'card-layout', isInverted = false }) => {
    ...
    return (React.createElement("div", { className: containerClasses, "data-testid": testId, key: layoutKey },
        React.createElement(ResponsiveGrid, { minItemWidth: minItemWidth, maxColumns: maxColumns, gap: gap, testId: `${testId}-grid` }, items.map((item, index) => (React.createElement(FeedItem, { key: `${item.link}-${index}`, item: item, variant: "card", showImage: !hideImages, showDescription: showDescription, showDate: showPubDate, showSource: showSource, imageAspectRatio: imageAspectRatio, fallbackImageUrl: fallbackImageUrl, forceFallback: forceFallback, onItemClick: handleItemClick, descriptionTruncation: {
                maxChars: truncateDescription,
                maxLines: 3
            }, className: styles.card, testId: `${testId}-item-${index}`, isInverted: isInverted }))))));
};
