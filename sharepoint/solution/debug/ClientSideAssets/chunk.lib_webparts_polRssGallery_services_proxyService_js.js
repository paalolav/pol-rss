"use strict";
(self["webpackJsonp_2b8f84b4-3b97-49d9-b352-9282b3ee05ae_1.3.0"] = self["webpackJsonp_2b8f84b4-3b97-49d9-b352-9282b3ee05ae_1.3.0"] || []).push([["lib_webparts_polRssGallery_services_proxyService_js"],{

/***/ 5237:
/*!*************************************************************!*\
  !*** ./lib/webparts/polRssGallery/services/proxyService.js ***!
  \*************************************************************/
...
/* harmony import */ var _utils_rssDebugUtils__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! ../utils/rssDebugUtils */ 3653);
/* harmony import */ var _rssSpecialFeedsHandler__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(/*! ./rssSpecialFeedsHandler */ 3522);
...
class ProxyService {
    ...
    static setTenantProxy(config) {
        ...
        if (config && this._debugMode) {
            _utils_rssDebugUtils__WEBPACK_IMPORTED_MODULE_2__.RssDebugUtils.log(`Tenant proxy configured: ${config.proxyUrl}`);
        }
    }
    /**
    ...
    static async testTenantProxy() {
        ...
        try {
            ...
            if (!response.ok) {
                if (this._debugMode) {
                    _utils_rssDebugUtils__WEBPACK_IMPORTED_MODULE_2__.RssDebugUtils.warn(`Tenant proxy health check failed: ${response.status}`);
                }
                return null;
            }
            const health = await response.json();
            if (this._debugMode) {
                _utils_rssDebugUtils__WEBPACK_IMPORTED_MODULE_2__.RssDebugUtils.log(`Tenant proxy health: ${health.status}, version: ${health.version}`);
            }
            return health;
        }
        catch (error) {
            if (this._debugMode) {
                ...
                _utils_rssDebugUtils__WEBPACK_IMPORTED_MODULE_2__.RssDebugUtils.warn(`Tenant proxy health check error: ${msg}`);
            }
            return null;
        }
    }
    ...
    static logError(method, error, url) {
        if (this._debugMode) {
            ...
            _microsoft_sp_core_library__WEBPACK_IMPORTED_MODULE_1__.Log.warn(this.LOG_SOURCE, `${method} failed for ${url}: ${errorMessage}`);
            _utils_rssDebugUtils__WEBPACK_IMPORTED_MODULE_2__.RssDebugUtils.warn(`ProxyService.${method} failed for ${url}: ${errorMessage}`);
        }
    }
    static async fetch(url, options = {}) {
        this._attemptedUrls = new Set();
        this._attemptedUrls.add(url);
        if (_rssSpecialFeedsHandler__WEBPACK_IMPORTED_MODULE_3__.RssSpecialFeedsHandler.isMeltwaterFeed(url)) {
        ...
        try {
            ...
            if (directResponse.ok) {
                if (this._debugMode) {
                    _utils_rssDebugUtils__WEBPACK_IMPORTED_MODULE_2__.RssDebugUtils.log(`Direct fetch succeeded for ${url}`);
                }
        ...
        if (ProxyService._tenantProxy) {
            try {
                ...
                if (tenantProxyUrl && !this._attemptedUrls.has(tenantProxyUrl)) {
                    ...
                    if (this._debugMode) {
                        _utils_rssDebugUtils__WEBPACK_IMPORTED_MODULE_2__.RssDebugUtils.log(`Trying tenant proxy: ${ProxyService._tenantProxy.proxyUrl}`);
                    }
                    ...
                    if (response.ok) {
                        if (this._debugMode) {
                            _utils_rssDebugUtils__WEBPACK_IMPORTED_MODULE_2__.RssDebugUtils.log(`Tenant proxy fetch succeeded`);
                        }
                        return response;
                    }
                    if (this._debugMode) {
                        _microsoft_sp_core_library__WEBPACK_IMPORTED_MODULE_1__.Log.info(this.LOG_SOURCE, `Tenant proxy returned ${response.status}.`);
                        _utils_rssDebugUtils__WEBPACK_IMPORTED_MODULE_2__.RssDebugUtils.log(`Tenant proxy returned ${response.status}, trying fallback proxies.`);
                    }
                }
            }
            catch (error) {
        ...
        for (const proxyUrl of ProxyService._proxyUrls) {
            try {
                ...
                if (this._attemptedUrls.has(proxiedUrl)) {
                    if (this._debugMode) {
                        _utils_rssDebugUtils__WEBPACK_IMPORTED_MODULE_2__.RssDebugUtils.warn(`Skipping already attempted proxy URL: ${proxiedUrl}`);
                    }
                    continue;
                }
                ...
                if (response.ok) {
                    if (this._debugMode) {
                        _utils_rssDebugUtils__WEBPACK_IMPORTED_MODULE_2__.RssDebugUtils.log(`Proxy fetch succeeded with ${proxyUrl}`);
                    }
                    return response;
                }
                if (this._debugMode) {
                    _microsoft_sp_core_library__WEBPACK_IMPORTED_MODULE_1__.Log.info(this.LOG_SOURCE, `Proxy ${proxyUrl} returned ${response.status}.`);
                    _utils_rssDebugUtils__WEBPACK_IMPORTED_MODULE_2__.RssDebugUtils.log(`Proxy ${proxyUrl} returned ${response.status}.`);
                }
            }
            catch (error) {
        ...
        if (ProxyService._httpClient) {
            try {
                ...
                const response = await ProxyService._httpClient.fetch(url, _microsoft_sp_http__WEBPACK_IMPORTED_MODULE_0__.HttpClient.configurations.v1, {
                    method: options.method || 'GET',
                    headers: headers
                });
                if (response.ok) {
                    if (this._debugMode) {
                        _utils_rssDebugUtils__WEBPACK_IMPORTED_MODULE_2__.RssDebugUtils.log(`SP HttpClient fetch succeeded for ${url}`);
                    }
                ...
                if (this._debugMode) {
                    _microsoft_sp_core_library__WEBPACK_IMPORTED_MODULE_1__.Log.info(this.LOG_SOURCE, `SP HttpClient returned ${response.status}.`);
                    _utils_rssDebugUtils__WEBPACK_IMPORTED_MODULE_2__.RssDebugUtils.log(`SP HttpClient returned ${response.status}.`);
                }
            }
            catch (error) {
        ...
        _microsoft_sp_core_library__WEBPACK_IMPORTED_MODULE_1__.Log.error(this.LOG_SOURCE, new Error(errorMsg));
    ...
    static async _fetchWithRedirectHandling(url, options) {
        ...
        while (redirectCount < this.MAX_REDIRECTS) {
            ...
            if (this._attemptedUrls.has(redirectUrl)) {
                if (this._debugMode) {
                    _utils_rssDebugUtils__WEBPACK_IMPORTED_MODULE_2__.RssDebugUtils.warn(`Detected redirect loop to already attempted URL: ${redirectUrl}`);
                }
                return response;
            }
            ...
            redirectCount++;
            if (this._debugMode) {
                _utils_rssDebugUtils__WEBPACK_IMPORTED_MODULE_2__.RssDebugUtils.log(`Following redirect ${redirectCount}/${this.MAX_REDIRECTS} to: ${currentUrl}`);
            }
        }
    ...
    static async fetchWithRetry(url, maxRetries = 3, options = {}) {
        ...
        for (let i = 0; i < maxRetries; i++) {
            try {
                ...
                if (response.ok) {
                    if (this._debugMode) {
                        _utils_rssDebugUtils__WEBPACK_IMPORTED_MODULE_2__.RssDebugUtils.log(`Direct fetch with retry ${i + 1} succeeded for ${url}`);
                    }
                    return response;
                }
                if (this._debugMode) {
                    _utils_rssDebugUtils__WEBPACK_IMPORTED_MODULE_2__.RssDebugUtils.warn(`Direct fetch with retry ${i + 1} returned ${response.status} for ${url}`);
                }
                ...
                if (this._debugMode) {
                    _utils_rssDebugUtils__WEBPACK_IMPORTED_MODULE_2__.RssDebugUtils.warn(`Direct fetch with retry ${i + 1} failed: ${lastError.message}`);
                }
            }
        ...
        try {
            if (this._debugMode) {
                _utils_rssDebugUtils__WEBPACK_IMPORTED_MODULE_2__.RssDebugUtils.log(`Falling back to proxy method after ${maxRetries} retries failed`);
            }
            const authInfo = this.extractAuthParams(url);
            const headersWithAuth = new Headers(options.headers || {});
            if (authInfo.apiKey) {
    ...
    static async fetchWithFirstProxy(url, options = {}) {
        ...
        if (this._debugMode) {
            _utils_rssDebugUtils__WEBPACK_IMPORTED_MODULE_2__.RssDebugUtils.log(`Trying first proxy: ${firstProxy}`);
        }
        const response = await this._fetchWithRedirectHandling(proxiedUrl, enhancedOptions);
        if (!response.ok) {
        ...
        if (this._debugMode) {
            _utils_rssDebugUtils__WEBPACK_IMPORTED_MODULE_2__.RssDebugUtils.log(`First proxy fetch succeeded`);
        }
        return response;
    }
}
...
//# sourceMappingURL=chunk.lib_webparts_polRssGallery_services_proxyService_js.js.map