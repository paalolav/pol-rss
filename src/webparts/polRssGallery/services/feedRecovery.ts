 ...
 */
const DEFAULT_STRATEGIES: RecoveryStrategyType[] = [
  'removeControlCharacters',
  'fixDuplicateDeclarations',
  'stripInvalidXml',
  'fixBrokenCdata',
 ...
 */
function fixUnclosedTags(xml: string): { content: string; fixes: number } {
  let content = xml;
  let fixes = 0;
  ...
  while ((match = tagRegex.exec(content)) !== null) {
    ...
    if (SELF_CLOSING_TAGS.includes(tagName)) continue;
 ...
 */
function fixBadEncoding(xml: string): { content: string; fixes: number } {
  let content = xml;
  let fixes = 0;
  ...
  content = content.replace(
    /encoding\s*=\s*["'](?:iso-8859-1|latin-?1|windows-1252)["']/gi,
    'encoding="UTF-8"'
  );
 ...
 */
function fixMissingNamespaces(xml: string): { content: string; fixes: number } {
  let content = xml;
  let fixes = 0;
...
function stripInvalidXml(xml: string): { content: string; fixes: number } {
  ...
  if (content.length !== beforeCtrl) fixes++;
 ...
 */
function extractFromHtml(xml: string): { content: string; fixes: number } {
  let content = xml;
  let fixes = 0;
 ...
 */
function fixBrokenCdata(xml: string): { content: string; fixes: number } {
  let content = xml;
  let fixes = 0;
 ...
 */
function removeControlCharacters(xml: string): { content: string; fixes: number } {
  let content = xml;
 ...
 */
function fixDuplicateDeclarations(xml: string): { content: string; fixes: number } {
  let content = xml;
  let fixes = 0;
 ...
 */
function fixMalformedAttributes(xml: string): { content: string; fixes: number } {
  let content = xml;
  let fixes = 0;
 ...
 */
function normalizeWhitespace(xml: string): { content: string; fixes: number } {
  let content = xml;
  let fixes = 0;
  ...
  if (content !== beforeCollapse) fixes++;
 ...
 */
function getStrategyDescription(strategy: RecoveryStrategyType): string {
  const descriptions: Record<RecoveryStrategyType, string> = {
 ...
 */
export function extractItemsAlternative(xml: string): Array<{
  title?: string;
  link?: string;
  description?: string;
  pubDate?: string;
  ...
  while ((itemMatch = itemPattern.exec(xml)) !== null) {
    ...
    const item: { title?: string; link?: string; description?: string; pubDate?: string } = {};
  ...
  while ((entryMatch = entryPattern.exec(xml)) !== null) {
    ...
    if (summaryMatch) item.description = summaryMatch[1].trim();
...
export function attemptRecovery(
  ...
  if (RssDebugUtils.isDebugEnabled() && result.recoveryAttempted) {
    RssDebugUtils.log(
      `Recovery completed: ${result.appliedStrategies.filter(s => s.success).length} strategies applied`
    );
  }
 ...
 */
export function validateRecoveredContent(xml: string): {
  valid: boolean;
  hasItems: boolean;
  itemCount: number;
  error?: string;
