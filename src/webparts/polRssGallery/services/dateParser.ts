 ...
 */
const MONTH_NAMES_NO: Record<string, number> = {
  jan: 0, januar: 0,
  feb: 1, februar: 1,
  mar: 2, mars: 2,
  apr: 3, april: 3,
 ...
 */
const ALL_MONTH_NAMES: Record<string, number> = {
  ...MONTH_NAMES,
  ...MONTH_NAMES_NO,
};
...
export function parseDate(dateStr: string): DateParseResult {
  ...
  const parsers: Array<{ format: DateFormat; parser: (s: string) => Date | null }> = [
 ...
 */
export function parseDateOrDefault(dateStr: string, fallback?: Date): Date {
 ...
 */
export function parseDateToIsoString(dateStr: string): string | null {
  const result = parseDate(dateStr);
  if (result.success && result.date) {
 ...
 */
function parseRfc3339(dateStr: string): Date | null {
 ...
 */
function parseIso8601(dateStr: string): Date | null {
 ...
 * Format: "2025-11-23"
 */
function parseIso8601DateOnly(dateStr: string): Date | null {
 ...
 */
function parseEuDate(dateStr: string): Date | null {
 ...
 */
function tryNativeDate(dateStr: string): Date | null {
  try {
 ...
 */
export function formatRelativeDate(date: Date, locale: string = 'nb-NO'): string {
  const now = new Date();
 ...
 */
export function compareDates(a: string, b: string, descending: boolean = true): number {
