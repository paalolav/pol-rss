...
export class UrlValidator {
  private allowedDomains: Set<string> | null = null;
  ...
  public validateFeedUrl(url: string): IUrlValidationResult {
    ...
    try {
      ...
      const hasIndicator = feedIndicators.some(
        (indicator) =>
          parsed.pathname.toLowerCase().includes(indicator) ||
   ...
   */
  private isNumericIp(hostname: string): boolean {
    // IPv4
   ...
   */
  private validateIpAddress(ip: string): IUrlValidationResult {
    // Parse IPv4
   ...
   */
  private isDomainInAllowlist(hostname: string): boolean {
    if (!this.allowedDomains) {
      return true;
    }
   ...
   */
  private checkSuspiciousPatterns(url: string): { warnings: string[] } {
    const warnings: string[] = [];
   ...
   */
  public static isValidFeedUrl(url: string): boolean {
    const validator = new UrlValidator();
