 ...
 */
export function validateImageUrl(url: string | null | undefined, baseUrl?: string): ValidatedUrl {
...
export function isImageMimeType(type: string | null | undefined): boolean {
  ...
  return normalizedType.startsWith('image/') ||
    ...
    ['jpeg', 'jpg', 'png', 'gif', 'webp', 'svg', 'bmp', 'ico'].some(ext =>
 ...
 */
function selectNamespacedElements(parent: Element, localName: string, namespace?: string): Element[] {
 ...
 */
export function extractMediaThumbnails(item: Element, options: ImageExtractionOptions = {}): ExtractedImage[] {
 ...
 */
export function extractMediaContent(item: Element, options: ImageExtractionOptions = {}): ExtractedImage[] {
  const images: ExtractedImage[] = [];
  const seenUrls = new Set<string>();
 ...
 */
export function extractEnclosureImages(item: Element, options: ImageExtractionOptions = {}): ExtractedImage[] {
  const images: ExtractedImage[] = [];
 ...
 */
export function extractImagesFromHtml(html: string | null | undefined, source: 'content:encoded' | 'description', options: ImageExtractionOptions = {}): ExtractedImage[] {
  if (!html) return [];
 ...
 */
export function extractItunesImage(item: Element, options: ImageExtractionOptions = {}): ExtractedImage | null {
 ...
 */
export function extractChannelImage(channel: Element | null | undefined, options: ImageExtractionOptions = {}): ExtractedImage | null {
  if (!channel) return null;

  // Try standard RSS channel image
  const imageElement = channel.querySelector('image > url');
  if (imageElement) {
    const url = imageElement.textContent?.trim();
    const validated = validateImageUrl(url, options.baseUrl);
    if (validated.isValid) {
      return {
        url: validated.url,
...
export function selectBestImage(
  ...
  preferredMinWidth: number = 0,
  preferredMinHeight: number = 0
): ExtractedImage | null {
 ...
 * 8. Configured fallback image
 */
export function extractImage(item: Element, options: ImageExtractionOptions = {}): ExtractedImage | null {
 ...
 */
export function extractImageUrl(item: Element, options: ImageExtractionOptions = {}): string | undefined {
