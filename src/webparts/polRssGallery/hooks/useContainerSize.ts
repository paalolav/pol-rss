...
const defaultOptions: Required<UseContainerSizeOptions> = {
  minItemWidth: 280,
  debounceMs: 0,
  defaultWidth: 0,
  defaultHeight: 0
...
export function useContainerSize(
  ...
  const calculateSize = useCallback((width: number, height: number): ContainerSize => {
    return {
      width,
      height,
  ...
  const [size, setSize] = useState<ContainerSize>(() =>
    calculateSize(opts.defaultWidth, opts.defaultHeight)
  );
 ...
 * ```
 */
export function useBreakpoint(containerRef: RefObject<HTMLElement>): BreakpointName {
 ...
 * ```
 */
export function useIsAtBreakpoint(
  containerRef: RefObject<HTMLElement>,
